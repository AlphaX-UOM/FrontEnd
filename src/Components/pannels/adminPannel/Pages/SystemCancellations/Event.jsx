...
export default function Event() {
    ...
    useEffect(() => {
      fetch(
        `https://alphax-api.azurewebsites.net/api/eventplannerservicereservations`    //`https://alphax-api.azurewebsites.net/api/eventplannerservicereservations/${userId}`
      )
        .then((response) => {
        ...
        .then((responseData) => {
            ...
            console.log("response data is approved? ->"+responseData[0].cancellation.isApproved);
        });
    }, []);
  ...
  return (
    ...
    <TableContainer component={Paper}>
      <Table className={classes.table} aria-label="customized table">
        ...
        <TableBody>
          {eventList.map((row) => (
            <StyledTableRow key={row.eventPlannerService.name}>
              <StyledTableCell component="th" scope="row">
                {row.eventPlannerService.name}
              </StyledTableCell>
              <StyledTableCell align="right">{row.eventPlannerService.userID}</StyledTableCell>
