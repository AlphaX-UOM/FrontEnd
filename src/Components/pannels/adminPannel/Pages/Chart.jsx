...
function BarGroup(props) {
    ...
    return <g className="bar-group">
      <text className="name-label" x="-70" y={yMid} alignmentBaseline="middle" >{props.d.name}</text>
  ...
  function BarChart()  {
    ...
    const [totalHotel,setTotalHote]=useState(0);
    const [hotel, sethotels] = useState([]);
    const [totalTransport,setTotalTransport]=useState(0);
    const [transport, setTransport] = useState([]);
    axios
      .get("https://alphax-api.azurewebsites.net/api/tourguideservicereservations")
      /*  .then((response) => {
      ...
      axios
      .get("https://alphax-api.azurewebsites.net/api/eventplannerservicereservations")
      /*  .then((response) => {
        return response.json();
      })*/
      .then((responseData) => {
      ...
      axios
      .get("https://alphax-api.azurewebsites.net/api/transportservicereservations")
      /*  .then((response) => {
        return response.json();
      })*/
      .then((responseData) => {
    ...
    const data=[
      ...
      return <svg width="1000" height="300" >
        <g className="container">
          <text className="title" x="20" y="30"> Reservations Per Services</text>
