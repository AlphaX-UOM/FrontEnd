        ...
        fetch(
            ...
            (data) => {
                ...
                if (userId!== undefined) {
                    ...
                    data = data.filter( (item) => item.userID === userId && item.tourGuideServiceID === props.add_id );
...
// console.log(ordered)
    ...
    const handleInputRatingPost = (event) => {
        ...
        event.preventDefault();
        var axios = require('axios');
        var data = JSON.stringify({ "id": ratingApi.id, "rating": value, "userID":userId,"tourGuideServiceID":props.add_id });
        var config = {
            method: 'post',
            url: `https://alphax-api.azurewebsites.net/api/tourguideServiceRatings/`,
            headers: {
                'Content-Type': 'application/json',
            },
            data: data
